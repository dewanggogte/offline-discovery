...
====================================================
Verify your Sarvam API key and test STT (saaras:v3) / TTS (bulbul:v3)
quality before running the full agent pipeline.
...
def test_tts():
    ...
    # Sample Hindi texts matching the agent's conversation style
    test_texts = [
        {
    ...
    for i, item in enumerate(test_texts):
        ...
        payload = {
            ...
            "speaker": "aditya",  # v3 male voice (matches agent_worker.py)
            "pace": 1.0,
            "loudness": 1.5,
            "speech_sample_rate": 24000,  # Browser quality
...
def test_llm():
    ...
    Note: The agent now uses Claude Haiku 3.5 or Qwen3 for LLM, not Sarvam.
    This test is kept for Sarvam API verification only."""
    print("\n🧠 Testing Sarvam Chat Completion (legacy — agent uses Claude/Qwen now)...")
    print("-" * 50)
